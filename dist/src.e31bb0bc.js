...
var questions = [{
  ...
  question: "Escriba sus apellidos",
  pattern: /^\S+\s\S+$/
}, {
  question: "Escriba su email",
  pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/
}, {
  question: "Escriba una contraseña",
  type: "password"
}];
...
(function () {
  ...
  function validate() {
    ...
    if (!inputField.value.match(questions[position].pattern || /.+/)) wrong();else ok(function () {
      progress.style.width = ++position * 100 / questions.length + 'vw';
      if (questions[position]) hideCurrent(putQuestion);else hideCurrent(done);
    });
  }
